...
- Build: `go build -o bin/dav-helper ./...`

## Daily hygiene (live address book)
- Inspect everything: `bin/dav-helper fetch`
- Force clients to refresh names/phones (REV bump): `bin/dav-helper fetch --touch-all`
- Add/update:
  - `bin/dav-helper add --name "Jane Doe" --emails jane@example.com --phones "+1 4803957551" --note "Friend"`
  - `bin/dav-helper update --name "Jane Doe" --new-name "Jane D." --phones "+1 4803957551,+91 9876543210"`
- Delete with safety net:
  - `bin/dav-helper delete --name "Noise Lead" --vcf "$UN_CONTACTS/psychology/noise-lead.vcf"`
...
- Photos:
  ...
  - Gravatar fallback: set `ENABLE_GRAVATAR=1`, then `bin/dav-helper photos --apply`

## Bucket hygiene (“un-contacts”)
- List buckets: `bin/dav-helper fetch --un-contacts`
- Move a server contact into a bucket (renaming optional):
  - `bin/dav-helper move --name "Old Vendor" --bucket corporate --new-name "Old Vendor (2019)"`
- Clean bucket numbers (ordering, E.164-ish): `bin/dav-helper clean-buckets --apply`
...
```
bin/dav-helper sync --source docs/examples/example-table.md --apply --touch
```
...
- `bin/dav-helper fix-names --apply`   # sets structured N=FN for all
- `bin/dav-helper refresh-uids --apply` # recreates cards with fresh UIDs/hrefs
- Then re-sync DAVx5/WhatsApp.

## Example “reclaim the life” loop
1) `bin/dav-helper fetch` and skim the table.
